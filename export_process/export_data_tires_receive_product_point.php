...
/*
$my_file = fopen("wheredate.txt", "w") or die("Unable to open file!");
fwrite($my_file, $where_date);
fclose($my_file);
*/
...
$filename = "Exp-Tires-" . date('m/d/Y H:i:s', time()) . ".csv";

@header('Content-type: text/csv; charset=UTF-8');
@header('Content-Encoding: UTF-8');
@header("Content-Disposition: attachment; filename=" . $filename);

$data = " วันที่ " . $_POST['doc_date_start'] . " ถึง " . $_POST['doc_date_to'] . "\n";
$data .= "\n";
$data .= "รหัสผู้ขาย,ชื่อผู้ขาย,เลขที่เอกสาร,วันที่,เดือน,ปี,ยี่ห้อ,รหัสสินค้า,รายละเอียด,ขนาดยาง,จำนวน,คะแนน (ต่อเส้น),คะแนน (รวม),ราคาต่อหน่วย,ราคารวม\n";
...
foreach ($tires_brand as $tr_brand) {
    ...
    foreach ($tires_size as $tr_size) {
        ...
        if ($tr_size === "R17") {
            $where_tires_size = " AND TRD_SH_NAME LIKE '%R17%' AND TRD_SH_NAME NOT LIKE  '%R17.5%' ";
        } else if ($tr_size === "R19") {
            $where_tires_size = " AND TRD_SH_NAME LIKE '%R19%' AND TRD_SH_NAME NOT LIKE  '%R19.5%' ";
        } else if ($tr_size === "R22") {
            $where_tires_size = " AND TRD_SH_NAME LIKE '%R22%' AND TRD_SH_NAME NOT LIKE  '%R22.5%' ";
        } else {
            $where_tires_size = " AND TRD_SH_NAME LIKE '%" . $tr_size . "%'";
        }

        $sql_tires = " SELECT AP_CODE,AP_NAME,DI_REF,DI_DATE,DI_MONTH_NAME,DI_YEAR,BRN_CODE,TRD_SH_CODE,TRD_SH_NAME,'" . $tr_size . "' AS TIRES_SIZE,TRD_QTY,TRD_U_PRC,TRD_G_KEYIN
                       FROM ims_product_receive_sac  
                       WHERE BRN_CODE = '" . $tr_brand . "'" . $where_tires_size
            // . " AND TRD_QTY > 0"
            . $where_date
        ...
        */
        ...
        foreach ($results_tires as $row_tires) {
            ...
            $sql_target = " SELECT * FROM ims_product_target_point_sac 
