...
if ($_POST["action"] === 'INIT') {
    ...
    $statement = $conn_cust->query($sql_get_main);
    ...
    foreach ($main_results as $main_result) {
        ...
        $statement = $conn_cust->query($sql_get_sub);
...
## Total number of records without filtering
    $stmt = $conn_cust->prepare("SELECT COUNT(*) AS allcount FROM ims_permission ");
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $conn_cust->prepare("SELECT COUNT(*) AS allcount FROM ims_permission WHERE 1 " . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $stmt = $conn_cust->prepare("SELECT * FROM ims_permission WHERE 1 " . $searchQuery
