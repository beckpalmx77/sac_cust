...
if ($_POST["action"] === 'ADD') {
    ...
    if ($_POST["email"] !== '') {
        ...
        if ($nRows > 0) {
            ...
            if ($lastInsertId) {
                Reorder_Record($conn_cust, "ims_user");
                echo 1;
            } else {
                echo 3;
            }
...
## Total number of records without filtering
    $stmt = $conn_cust->prepare("SELECT COUNT(*) AS allcount FROM ims_user ");
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $conn_cust->prepare("SELECT COUNT(*) AS allcount FROM ims_user WHERE 1 " . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $stmt = $conn_cust->prepare("SELECT * FROM ims_user WHERE 1 " . $searchQuery
