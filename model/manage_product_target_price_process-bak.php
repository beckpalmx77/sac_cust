...
if ($_POST["action"] === 'ADD') {
    if ($_POST["name_t"] !== '') {
        $product_id = "S-" . sprintf('%04s', LAST_ID($conn_sac, "ims_product_price", 'id'));
        $name_t = $_POST["name_t"];
        ...
        if ($nRows > 0) {
            ...
            $sql = "INSERT INTO ims_product_price(product_id,name_t,price_year,point_1,point_2) 
            VALUES (:product_id,:name_t,:price_year,:point_1,:point_2)";
...
if ($_POST["action"] === 'UPDATE') {
    if ($_POST["product_id"] != '') {
        ...
        $point_1 = $_POST["point_1"];
        $point_2 = $_POST["point_2"];
        $sql_find = "SELECT * FROM ims_product_price WHERE product_id = '" . $product_id . "'";
        $nRows = $conn_sac->query($sql_find)->fetchColumn();
        if ($nRows > 0) {
            $sql_update = "UPDATE ims_product_price SET name_t=:name_t
            ,price_year=:price_year,point_1=:point_1,point_2=:point_2
            WHERE id = :id";
...
if ($_POST["action"] === 'GET_PRODUCT_TIRES') {

    $my_file = fopen("SEARCH_DATA-1.txt", "w") or die("Unable to open file!");
    fwrite($my_file, "GET_PRODUCT_TIRES");
    fclose($my_file);
